#simple gui calc
#if u copying, 61 lines without comments and spaces :) :P
from tkinter import *
...
if __name__ == "__main__":
	...
	plus = Button(gui, text=' + ', fg='black',command=lambda: press("+"), height=1, width=7)
	plus.grid(row=2, column=3)

	minus = Button(gui, text=' - ', fg='black',command=lambda: press("-"), height=1, width=7)
	minus.grid(row=3, column=3)

	multiply = Button(gui, text=' * ', fg='black',command=lambda: press("*"), height=1, width=7)
	multiply.grid(row=4, column=3)

	divide = Button(gui, text=' / ', fg='black',command=lambda: press("/"), height=1, width=7)
	divide.grid(row=5, column=3)

	equal = Button(gui, text=' = ', fg='black',command=equalpress, height=1, width=7)
